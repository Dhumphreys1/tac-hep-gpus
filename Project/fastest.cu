...
int main(void){
	...
	float *h_A, *h_B, *h_A_stenciled, *h_B_stenciled;
    float *d_A, *d_B, *d_A_stenciled, *d_B_stenciled, *result;
    ...
    cudaMallocManaged(&result, num_elements*sizeof(float));
    cudaCheckErrors("Failed to allocate result");
    ...
    matrix_mul_fast<<<dimGrid, dimBlock, 0, matrix_mult>>>(d_A_stenciled, d_B_stenciled, result);
    ...
    printf("Fastest total execution time: %ld ms\n", duration.count());
    return 0;
}